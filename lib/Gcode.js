export default class Gcode {
	constructor (config = {}) {
		this.maxSize = config && config.maxSize || {width: 200, height: 287}
	...
	generate (position) {
		...
		for (var i = 0; i < position.length ; i++) {
			...
			var penState = (position[i].penState || 'down')
			if (this.lastPenState != penState) this.gcodeStr += position[i].penState === 'down' ? this.penDownCommand : this.penUpCommand
			this.gcodeStr += `${this.baseCommand} X${maxPos.x * this.revert} Y${maxPos.y * this.revert} \n` 
			this.lastPenState = penState
		}	
		this.backToZero()
		return this.gcodeStr
	}
	getMinMaxPos (pos) {
		...
		return {
			x: Math.min(noNegative.x, this.maxSize.width),
			y: Math.min(noNegative.y, this.maxSize.height)
		}
	}
